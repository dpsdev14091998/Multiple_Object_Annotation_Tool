...
def main(fname):
	...
	with open('manage.json', 'w') as fl:
		x1={}
		x1["license"]="1"
		x1["url"]="http://35.174.150.191:8007/server_files/pe_tagging_nov26/000680.jpg"
		x1["file_name"]="000680.jpg"
		x1["height"]=432
		x1["id"]=100
		images.append(x1)
		...
		if k==ord('c'):
			...
			with open("manage.json",'w') as fl:
				for key in d:
					...
					user["num_keypoints"]=17
				
					user["keypoints"]= d[key][0:17]
					
					bb_box.append(d[key][18][0])
					bb_box.append(d[key][19][0])
					bb_box.append(d[key][18][1])
					bb_box.append(d[key][19][1])

					bb_box= [float(item) for item in bb_box]
					user["bound"]=bb_box
					user["image_id"]=id_img
